package net.akat.api.spigui.menu;

import net.akat.api.spigui.SpiGUI;
import net.akat.api.spigui.buttons.SGButton;
import net.akat.api.spigui.toolbar.SGToolbarBuilder;
import net.akat.api.spigui.toolbar.SGToolbarButtonType;
...
public class SGMenuListener implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (clickedInventory == null) {
            return; // If it's null, exit the method early
        }

        // Get the instance of the SpiGUI that was clicked.
        SGMenu clickedGui = (SGMenu) clickedInventory.getHolder();

        // If clickedGui is null (the inventory does not belong to an SGMenu), return early.
        if (clickedGui == null) {
            return;
        }
        ...
        // do nothing else.
        if (clickedGui.getPermittedMenuClickTypes() != null &&
                clickedGui.getPermittedMenuClickTypes().stream().noneMatch(type -> type == event.getClick())) {
            event.setResult(Event.Result.DENY);
            return;
        }

        // If the action is blocked, instantly deny the event
        if (clickedGui.getBlockedMenuActions() != null &&
                clickedGui.getBlockedMenuActions().stream().anyMatch(action -> action == event.getAction())) {
            event.setResult(Event.Result.DENY);
            return;
        }

        // Check if the GUI is owned by the current plugin
        ...
        // Handle pagination and other actions
    ...
    public void onAdjacentInventoryClick(InventoryClickEvent event) {
        ...
        if (view.getTopInventory() == null || shouldIgnoreInventoryEvent(view.getTopInventory())) return;
        if (event.getClickedInventory() == view.getTopInventory()) return;

        Inventory clickedInventory = event.getClickedInventory();
        if (clickedInventory == null) return;

        if (!(view.getTopInventory().getHolder() instanceof SGMenu clickedGui)) return;

        // ✅ Проверка на null перед вызовом stream()
        Set<InventoryAction> blockedActions = clickedGui.getBlockedAdjacentActions();
        if (blockedActions != null && blockedActions.stream().anyMatch(action -> action == event.getAction())) {
            event.setResult(Event.Result.DENY);
        }
    }
